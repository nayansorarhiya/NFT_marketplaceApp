...
export default function CartDrawer({
  ...
  const buyNow = async () => {
    ...
    try {
      ...
      const diamondContract = await getDiamondContract(
        contractaddr,
        diamondswapABI,
        signer
      );
      let token = apifilter.buy.filter(
        (ele) =>
      ...
      ERC1155 = ERC1155.map((ele) => {
        return [ele.address, ele.tokenId, ele.amount];
      });
      ERC721 = ERC721.map((ele) => {
        return [ele.address, ele.tokenId];
      });
    ...
    <>
      ...
      >
        ...
        <Box sx={{ padding: "30px 20px 20px 20px" }}>
            ...
            {CartData.map((value) => {
              return (
                <CartItems
                  ...
                  usdprice={usdprice}
                  roundOff={roundOff}
                  popCartData={popCartData}
                ></CartItems>
              );
            })}
          </Box>

          {CartData.length !== 0 && active && (
            <>
          ...
          {active && CartData.length !== 0 && balance.lt(totalamount) && (
            <Button
              onClick={() => !active && ConnectModal()}
              sx={{
                mt: 3,
            ...
            <>
              <Box sx={{ mt: 3 }}>
                <Button
                  sx={{
                    background:
                      "linear-gradient(90deg, #E875D2 0%, #642CC8 100%)",
                    borderRadius: "3px",
                    padding: "11px 22px",
                    color: "#ffffff",
                    width: "100%",
